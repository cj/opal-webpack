...
var LOAD_PATH = process.env.OPAL_LOAD_PATH ? process.env.OPAL_LOAD_PATH.split(':') : [process.cwd()]

if (process.env.OPAL_USE_BUNDLER) {
  // these quotes are fine
  // eslint-disable-next-line
  const opalPaths = bundlerExecute('ruby -e "require \'opal\'; puts Opal.paths"').toString().trim().split("\n")
  LOAD_PATH = opalPaths.concat(LOAD_PATH)
}
...
module.exports = function resolveFilename(filename) {
  ...
  if (!process.env.OPAL_USE_BUNDLER && opalRequiresForStubbing.indexOf(filename) != -1) {
    return {
